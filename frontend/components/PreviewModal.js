...
function getPreviewType(link) {
  ...
  // Upload video → show inline video player
  if (link.source === 'upload' && link.file_path && /\.(mp4|mov|webm)$/i.test(link.file_path)) {
    return 'local-upload-video';
  }
...
export default function PreviewModal({ link, onClose }) {
  ...
  // Build thumbnail src
  // Video uploads: thumbnail is generated by ffmpeg into THUMB_DIR → /files/thumbnails/
  // Image uploads: thumbnail IS the image itself, stored in UPLOAD_DIR → /files/uploads/
  const isVideoUpload = link.source === 'upload' && /\.(mp4|mov|webm)$/i.test(link.file_path || '');
  const thumbSrc = link.local_thumbnail
    ? (link.source === 'upload' && !isVideoUpload
